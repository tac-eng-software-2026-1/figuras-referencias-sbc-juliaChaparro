...
\author{Julia Evelyn Chaparro Ferreira\inst{1}}

\address{Instituto de Computação -- Ufam \\
  Manaus -- Amazonas -- Brasil
  \email{julia.ferreira@icomp.ufam.edu.br}
}
...
\begin{abstract}
Este artigo apresenta um exemplo simples de uso do template da SBC
no Overleaf, incluindo inserção de figura, uso de citações bibliográficas e arquivo .bib.
\end{abstract}

\begin{resumo}
Este documento mostra como estruturar um texto acadêmico em \LaTeX{}
no formato da SBC, inserindo imagem, referências bibliográficas e citações no
corpo do texto.
\end{resumo}
...
\caption{Imagem de exemplo (cameraman)}
\label{fig:cameraman}
\end{figure}

Como discutido por \cite{lamport1994}, o uso correto de elementos gráficos melhora
a clareza e a apresentação do texto acadêmico.

\section{Referências e Citações}

As citações bibliográficas podem ser feitas com o comando \verb|\cite|. Por exemplo,
podemos citar um livro clássico de \LaTeX{} \cite{lamport1994} e também a obra de
...
Este exemplo apresentou a estrutura básica de um artigo no formato SBC com inclusão
de figura, uso de citações e arquivo bibliográfico externo. Esse tipo de organização é
útil para relatórios, artigos e trabalhos acadêmicos em Computação.
